...
fire_sound = pygame.mixer.Sound("Big Bomb.wav")
explosion_sound = pygame.mixer.Sound("Big Bomb.wav")
...
medfont = pygame.font.SysFont("stencil", 50)
largefont = pygame.font.SysFont("Showcard Gothic", 75)
...
def game_controls():
    ...
    background = pygame.image.load("tank_img2.png") ########################
    gcont = True
        ...
        for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_c:
                        paused = False
                        pygame.mixer.music.load("Battle Finale.wav")
...
def level(input1):
    if input1=="easy":
        ...
        player_health1=100
        enemy_health2=50
        return(level_tough1,level_tough,player_health1,enemy_health2)
    if input1=="medium":
        level_tough1=1.1
        level_tough=0.9
        player_health1=100
...
def e_fireShell(xy,tankx,tanky,turPos,gun_power,xlocation,barrier_width,randomHeight,ptankx,ptanky,level_tough,level_tough1):
    pygame.mixer.Sound.play(fire_sound)
    damage = 0
    currentPower = 1
    power_found = False
    ...
    while fire:
        ...
        gun_power = random.randrange(int(currentPower*level_tough), int(currentPower*level_tough1))#####################################level tough
...
def game_intro():#################################################################################
    pygame.mixer.music.play(-1)###################################
    background = pygame.image.load("tank_img.png")              ###background image
    intro = True
    ...
    while intro:
        ...
        gameDisplay.blit(background,[0,0])                        ###background image         
        message_to_screen("Welcome to Tanks!",black,-100,size="large")
        ...
        button("next", 150,500,100,50, green, light_green, action="next")
        button("controls", 350,500,135,50, yellow, light_yellow, action="controls")
        button("quit", 550,500,100,50, red, light_red, action ="quit")
...
def game_intro1():#################################################################################
    background = pygame.image.load("tank_img2.png")              ###background image
    intro1 = True
    level1="medium"
    while intro1:
        for event in pygame.event.get():
        ...
        gameDisplay.fill(white)
        gameDisplay.blit(background,[0,0])                        ###background image         
        message_to_screen("Choose the difficulty:",
                          black,
                          -100,
                          size="medium")
...
def game_over():
    ...
    game_over = True
    background = pygame.image.load("tank_img3.png") ########################
    while game_over:
        for event in pygame.event.get():
                #print(event)
                if event.type == pygame.QUIT:
        ...
        message_to_screen("You died.",white,-30,size="medium")

        button("play Again", 150,500,150,50, green, light_green, action="next")
        button("controls", 350,500,135,50, yellow, light_yellow, action="controls")
        button("quit", 550,500,100,50, red, light_red, action ="quit")
...
def you_win():
    ...
    win = True
    background = pygame.image.load("tank_img4.png") ########################
    while win:
        for event in pygame.event.get():
                #print(event)
                if event.type == pygame.QUIT:
        ...
        gameDisplay.blit(background,[0,0])#############################
        message_to_screen("You won!",black,-100,size="large")
...
def gameLoop(level1):
    ...
    level_tough1,level_tough,player_health1,enemy_health2=level(level1)
                ...
                if event.key == pygame.K_LEFT:
                    ...
                    for x in range(random.randrange(0,10)):
                        ...
                        if display_width * 0.3 > enemyTankX > display_width * 0.03:
                            print("enemytankx",display_width * level_tough1)
                            if possibleMovement[moveIndex] == "f":
                                enemyTankX += 5
                            elif possibleMovement[moveIndex] == "r":
                                enemyTankX -= 5

                            background = pygame.image.load("sky_cloud.png") ########################
                            gameDisplay.blit(background,[0,0])#############################
                    ...
                    damage = e_fireShell(enemy_gun,enemyTankX,enemyTankY,8,50,xlocation,barrier_width,randomHeight,mainTankX,mainTankY,level_tough,level_tough1)
                    player_health1 -= damage
